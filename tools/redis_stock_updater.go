...
// UpdateProductStockInRedisSync updates the product stock in the temporary cache with branch and user context synchronously
func UpdateProductStockInRedisSync(cacheKey, productID string, newStock int) {
	ctx := context.Background()
	// Ping Redis to check connection
	if _, err := config.RDB.Ping(ctx).Result(); err != nil {
		fmt.Printf("Redis ping failed: %v\n", err)
...
// UpdatePurchaseProductStockInRedisSync updates the purchase product stock in the temporary cache with branch and user context synchronously
func UpdatePurchaseProductStockInRedisSync(cacheKey, productID string, newStock int) {
	ctx := context.Background()
	// Ping Redis to check connection
	if _, err := config.RDB.Ping(ctx).Result(); err != nil {
		fmt.Printf("Redis ping failed: %v\n", err)
